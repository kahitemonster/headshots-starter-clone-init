...
// const leapImageWebhookUrl = `http://${process.env.VERCEL_URL}/leap/image-webhook`;
const leapWebhookSecret = process.env.LEAP_WEBHOOK_SECRET;
// const stripeIsConfigured = process.env.NEXT_PUBLIC_STRIPE_IS_ENABLED === "true";
const stripeIsConfigured = true

const prompts = "8k close up linkedin profile picture of @subject {model_type}"
  ...
  try {
    if (result.status === "finished") {
      ...
      const { status, statusText } = await leap.images.generate({
        prompt: prompts.replace("{model_type}", (model_type as string) ?? ""),
        numberOfImages: 2,
        height: 512,
        width: 512,
        steps: 50,
